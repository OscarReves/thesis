...
class HNSWRetriever:
    def __init__(self, index_path, documents, device=None, text_field='text'):
        ...
        # Load hnswlib index
        self.index = hnswlib.Index(space='cosine', dim=1024)  # Use actual dim if known
    ...
    def retrieve(self, questions, top_k=5):
        q_embs = self.embed([f"query: {q}" for q in questions])
        I, _ = self.index.knn_query(q_embs, k=top_k)
        return [
            "\n\n".join([self.contexts[idx] for idx in indices])
            for indices in I
        ]
